...
def aggregate_and_evaluate_predictions(filepath, ground_truth_dict):
    predictions = load_json(filepath)
    avg_f1 = []
    avg_facts_f1 = []
    ...
    for pair_id, pair_truth in ground_truth_dict.items():
        ...
        for pred in predictions:
            if pred['pair_id'].startswith(pair_id):
        ...
        if len(found_predictions) > 1:
            ...
            if 'predicted_evidence' in predictions[0]:
                ...
                for pred in found_predictions:
                    ...
                    difference = len(predicted_facts) - len(pred_facts_mask)
                    assert difference >= 0
                    if difference > 0:
...
if __name__ == '__main__':
    ...
    for folder in tqdm(os.listdir(predictions_path)):
        sub_path = os.path.join(predictions_path, folder, 'predictions.json')
        model_f1, model_facts_f1 = aggregate_and_evaluate_predictions(sub_path, ground_truth_dict)
